...
def get_report(ds_overlay, output_dict):
    ...
    if keyworderror_hit:
        output_dict['Comments'] = combine_str(output_dict.get('Comments'), 'Keyword error. Could not find ', tag_name)
        tag_name = ''
    if not encuentro_ROI:
        output_dict['Comments'] = combine_str(output_dict.get('Comments'), 'ROI values not found')
    if encuentro_ROI and not keyworderror_hit:
        output_dict['Comments'] = combine_str(output_dict.get('Comments'), 'No comments.') 
    return output_dict

def parse_time(_time):
    my_time = time.strptime(_time, '%H%M%S.%f')
    return time.strftime('%H:%M:%S', my_time)
...
def main():
    ...
    #TODO: solucionar esto de una mejor manera para hacerlo más versatil a meter argumentos
