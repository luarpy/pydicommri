...
def compute_fat(in_phase_path, opp_phase_path, output_mha_path):
  ...
  fat_arr = 0.5 * np.maximum(in_phase_arr - opp_phase_arr, 0)
  
  # Create a new SimpleITK image from the fat array
  fat_image = sitk.GetImageFromArray(fat_arr)
  fat_image.CopyInformation(in_phase)

  # Write the fat image to the output MHA file
  sitk.WriteImage(fat_image, output_mha_path)
